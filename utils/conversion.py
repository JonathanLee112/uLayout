"""
This code is from the LGTNET repository. https://github.com/zhigangjiang/LGT-Net
"""
...
def pixel2uv(pixel, w=1024, h=512, axis=None):
    pixel = pixel.astype(np.float16) if isinstance(pixel, np.ndarray) else pixel.float()
...
def uv2pixel(uv, w=1024, h=512, axis=None, need_round=True):
    ...
    lst = [pu, pv]
    if need_round:
        pixel = np.concatenate(lst, axis=-1).round().astype(np.int16) if isinstance(uv, np.ndarray) else torch.cat(lst,
                                                                                                                 dim=-1).round().int()
    else:
