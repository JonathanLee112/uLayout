...
class BaseModule(nn.Module):
    def __init__(self, ckpt_dir=None):
        ...
        self.last_pp_model_path = None
        self.best_pp_model_path = None
        self.best_pp_accuracy = -float('inf')
        self.acc_d = {}
    ...
    def save(self, optim, epoch, accuracy, logger, replace=True, acc_d=None, config=None, acc_index=None):
        """
        ...
        name = datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S_last_{}_{:.4f}_{}'.format(acc_index, accuracy, epoch))
        ...
        if 'pp' in acc_index:
            ...
            if (True or config.MODEL.SAVE_LAST) and epoch % config.TRAIN.SAVE_FREQ == 0:
                pp_name = f"pp_{name}"
                if replace and self.last_pp_model_path and os.path.exists(self.last_pp_model_path):
                    os.remove(self.last_pp_model_path)
                self.last_pp_model_path = os.path.join(self.ckpt_dir, pp_name)
                torch.save(checkpoint, self.last_pp_model_path)
                logger.info(f"Saved last pp model: {self.last_pp_model_path}")

            if accuracy > self.best_pp_accuracy:
                self.best_pp_accuracy = accuracy
                if True or config.MODEL.SAVE_BEST:
                    pp_name = f"pp_model_{name.replace('last', 'best')}"
                    if replace and self.best_pp_model_path and os.path.exists(self.best_pp_model_path):
                        os.remove(self.best_pp_model_path)
                    self.best_pp_model_path = os.path.join(self.ckpt_dir, pp_name)
                    torch.save(checkpoint, self.best_pp_model_path)
                    logger.info("#" * 100)
                    logger.info(f"Saved best pp model: {self.best_pp_model_path}")
                    logger.info("#" * 100)
        else:
            if (True or config.MODEL.SAVE_LAST) and epoch % config.TRAIN.SAVE_FREQ == 0:
