...
class PanoSt2D3DDataset(data.Dataset):
    def __init__(self, root_dir, mode='train', subset='pano',
        ...
        self.img_fnames = sorted([
            ...
            if fname.startswith(keywords) and (fname.endswith('.jpg') or fname.endswith('.png'))
        ])
        self.txt_fnames = sorted([
            fname for fname in os.listdir(self.cor_dir)
            if fname.startswith(keywords) and fname.endswith('.txt')
        ])
